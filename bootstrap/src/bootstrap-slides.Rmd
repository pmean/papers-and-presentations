...
Then I will show two simple examples.
...
### Compute confidence intervals, BCa

+ Bias adjustment (using bootstrap estimates)
  + $\hat z_0=\Phi^{-1}(\frac{1}{B}H(\hat\theta^{(b)}-\hat\theta))$
...
If a-hat is equal to zero, there is no adjustment to the confidence interval based on skewness.
...
### Percentiles with no adjustment
...
### Percentiles with bias adjustment
...
### Percentiles with acceleration adjustment
...
+ Test
  + $H_0:\ \theta=0$
  + $H_1:\ \theta>0$
+ Reject $H_0$ if more than 1-$\alpha$ of the $\hat\theta^{(b)}$'s are greater than 0.
