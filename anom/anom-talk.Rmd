...
data.frame(x=x, y=y) %>%
  ggplot(aes(x, y)) +
    expand_limits(x=c(25, 45)) +
    geom_point() +
    ylab(" ") +
    geom_segment(x=lo, y=y, xend=hi, yend=y) +
    scale_y_continuous(breaks=1:4, minor_breaks=NULL, labels=LETTERS[1:4])
```
...
data.frame(workers=1:8, average_ore_produced) %>%
  ...
  geom_hline(yintercept=mn-3*5, linetype="dashed") +
  scale_x_continuous(breaks=1:8, minor_breaks=NULL, labels=worker_names)
```
...
The deviations for Position 1 are small but mostly negative.
...
c_section_rates %>%
  ...
  scale_x_continuous(
    breaks=1:29,
    minor_breaks=NULL,
    labels=c_section_rates$group) -> p1
plot(p1)
```  

<div class="notes">

Here is a plot of the proportions.
...
p1 +
  ...
  geom_segment(x=(1:29)+0.5, y=c(lo[-1], hi[29]), xend=(1:29)+0.5, yend=lo)
```
...
There are `r N` CAT scans and `r format(M, big.mark=",")` members across all clinics, providing an overall rate of `r ubar` scans per thousand patients.

<div class="notes">

This example is taken from page 46 of a book by Nelson, Wludyka, and Copeland. CAT scans are done at five different clinics. Each clinic serves a different number of members. 
...
rates %>%
  ggplot(aes(x=clinic, y=rate_per_thousand)) +
    ...
    geom_segment(x=clinic, y=ubar, xend=clinic, yend=rate_per_thousand) -> uchart
uchart
```
...
uchart +
  ...
  geom_segment(x=clinic-0.5, y=hi, xend=clinic+0.5, yend=hi)
```
